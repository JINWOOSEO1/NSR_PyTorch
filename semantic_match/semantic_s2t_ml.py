...
def get_input(mesh_file):
    positions, ss = mesh_to_voxels(mesh_file, coarse_resolution * 4)  # positions: (N, 3)
    ss = ss[None].float()
    ss = ss.cuda().float() 
...
# Layer 4: Global semantic features
...
_, source_latent = get_input(source_dir) # (1, 8, 16, 16, 16)
...
chosen_voxel_index = np.where((source_coords == chosen_voxel).all(axis = 1))[0] # voxel indices(64) which have same voxel index(16)
source_colormap[chosen_voxel_index] = [0,0,0] # black color

color_ply_with_colormap(source_position, source_colormap, output_root_path, name='colored_source.ply')

render_num = 10

source_images = get_render_imgs(source_image_dir, render_num)
source_features = get_features(source_images, source_latent, noise_d=noise_d, extract_t=extract_t)

target_image_path =  os.path.join(target_dir, 'renders')
target_positions, target_latent = get_input(target_dir)
target_coords = ((torch.tensor(target_positions) + 0.5) * coarse_resolution).int().contiguous()
...
# Step 1: CGlobal Initialization(layer 4)
...
for layer_idx in range(1, len(layers_for_matching)):
    ...
    current_source = feature_to_3d(source_features[current_layer], reso=coarse_resolution) # (1, 1024, 16, 16, 16)
    current_target = feature_to_3d(target_features[current_layer], reso=coarse_resolution) # (1, 1024, 16, 16, 16)
...
target_map_index = np.where((target_coords == target_matched_voxel).all(axis=1))[0]
print(f"Number of matched points in point cloud: {len(target_map_index)}")

target_colormap = np.full((len(target_coords), 3), 0.7) 
